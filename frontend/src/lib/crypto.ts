...
export async function encryptPaste(
  ...
  const compressed = compressionMode === "zlib" ? await compress(textBytes) : textBytes;
...
export async function encryptComment(
  ...
  password: string,
  compressionMode: string = "zlib"
): Promise<{ ct: string; adata: CommentAdata }> {
  const iv = crypto.getRandomValues(new Uint8Array(16));
  const salt = crypto.getRandomValues(new Uint8Array(8));

  const adata: CommentAdata = [
    uint8ToBase64(iv),
    uint8ToBase64(salt),
    100000,
    256,
